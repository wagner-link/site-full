...
import { CreateEventDialog } from "@/features/event/presentation";
                  ...
                  return (
                    <div key={hour} className={cn("relative", isDisabled && "bg-calendar-disabled-hour")} style={{ height: "96px" }}>
                      ...
                      <DroppableTimeBlock date={selectedDate} hour={hour} minute={0}>
                        <CreateEventDialog defaultStartDate={selectedDate} defaultStartTime={`${hour.toString().padStart(2, '0')}:00`}>
                          <div className="absolute inset-x-0 top-0 h-[24px] cursor-pointer transition-colors hover:bg-accent" />
                        </CreateEventDialog>
                      </DroppableTimeBlock>

                      <DroppableTimeBlock date={selectedDate} hour={hour} minute={15}>
                        <CreateEventDialog defaultStartDate={selectedDate} defaultStartTime={`${hour.toString().padStart(2, '0')}:15`}>
                      ...
                      <DroppableTimeBlock date={selectedDate} hour={hour} minute={30}>
                        <CreateEventDialog defaultStartDate={selectedDate} defaultStartTime={`${hour.toString().padStart(2, '0')}:30`}>
                          <div className="absolute inset-x-0 top-[48px] h-[24px] cursor-pointer transition-colors hover:bg-accent" />
                        </CreateEventDialog>
                      </DroppableTimeBlock>

                      <DroppableTimeBlock date={selectedDate} hour={hour} minute={45}>
                        <CreateEventDialog defaultStartDate={selectedDate} defaultStartTime={`${hour.toString().padStart(2, '0')}:45`}>
